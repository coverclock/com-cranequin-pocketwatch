...
* That's it. The board comes up in almost no time at all after power up. Use the S1 and S3 buttons to non-persistently set your local time zone (which is indicated using the NATO nomenclature, e.g. Z or "Zulu" for UTC, T or "Tango" for my own MST), and the S2 button to enable (or disable) the use of Daylight Saving Time (DST) if your state uses it (not all do).
# Contact
John Sloan    
Cranequin LLC   
